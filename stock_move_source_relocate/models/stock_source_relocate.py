...
class StockSourceRelocate(models.Model):
    ...
    def _rule_for_move(self, move):
        ...
        # fast. Searching all the parent locations then filtering the rules
        # matching them can be much slower if we have many locations.
        rules = self.search(
            [("picking_type_id", "=", move.picking_type_id.id)]
        ).filtered(
            lambda rule: move.location_id.is_sublocation_of(rule.location_id)
            and rule._is_rule_applicable(move)
        )
